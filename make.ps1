...
Function Set-Version($path, $apiVersion, $majorVersion, $minorVersion, $patchVersion, $schemaVersion) {
    $semanticVersion = '{0}.{1}.{2}' -f $majorVersion, $minorVersion, $patchVersion
    $packedVersion = '{0:D2}{1:D2}{2:D2}' -f $majorVersion, $minorVersion, $patchVersion
    Write-Verbose (
        'Setting ' + $path + ' to api ' + $apiVersion + ', version ' + $semanticVersion + ' (' +  $packedVersion + '), schema ' + $schemaVersion
    )
    ForEach ($file in Get-ChildItem -Path $path -File) {
        ...
        (Get-Content $target).Replace('[SEMANTIC_VERSION]', $semanticVersion) | Set-Content $target
        (Get-Content $target).Replace('[PACKED_VERSION]', $packedVersion) | Set-Content $target
...
If ($Build) {
    ...
    ForEach ($dir in Get-ChildItem -Path $buildDir -Directory) {
        ...
        $addOnZipFile = $distDir + '\' + $dir.Name + '-' + $packedVersion + '.zip'
